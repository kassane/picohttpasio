...
private:
    void readRequest() {
        ...
        asio::async_read_until(socket_, asio::dynamic_buffer(request_data_), "\r\n\r\n",
                               [this, self](std::error_code ec, std::size_t bytes_transferred) {
            if (!ec) {
                std::string request_str = request_data_.substr(0, bytes_transferred);
                std::string response;
...
class HTTPServer {
public:
    HTTPServer(asio::io_context& io_context, short port, const std::string& root_dir)
        : acceptor_(io_context, tcp::endpoint(tcp::v4(), port)), requestHandler_(root_dir) {
        startAccept();
    }
...
private:
    void startAccept() {
        acceptor_.async_accept([this](std::error_code ec, tcp::socket socket) {
            if (!ec) {
                std::make_shared<HTTPServerConnection>(std::move(socket), requestHandler_)->start();
            }
            startAccept();
        });
    }
